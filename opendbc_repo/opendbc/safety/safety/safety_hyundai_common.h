...
extern bool hyundai_joystick_debug;
bool hyundai_joystick_debug = false;
...
void hyundai_common_init(uint16_t param) {
  ...
  const int HYUNDAI_PARAM_JOYSTICK_DEBUG = 1024;  // Joystick debug mode
  ...
  hyundai_joystick_debug = GET_FLAG(param, HYUNDAI_PARAM_JOYSTICK_DEBUG);
...
void hyundai_common_cruise_state_check(const bool cruise_engaged) {
  ...
  if (!hyundai_longitudinal) {
    ...
    // 🚨 Joystick debug mode: Keep controls_allowed even when cruise is off
    if (!cruise_engaged && !hyundai_joystick_debug) {
